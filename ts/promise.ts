...
export default class PromiseA implements IPromise{
    ...
    constructor(executor:(resolve:(value?:any)=>void,reject:(reason?:any)=>void)=>void){
        ...
        function resolvePromise(value?:any):any{
            ...
            return executeFullfill(PromiseStates.fullfilled,value);
        }
        function rejectPromise(reason?:any):any{
            if(promise.__promise_status !== PromiseStates.padding) {
                //console.warn("promise rejected/resolved more than once.",promise,this);
                return promise;
            }
            return executeFullfill(PromiseStates.rejected,reason);
        }

        function executeFullfill(status:PromiseStates,value:any):PromiseA{
            ...
            _async(function(){
                ...
                fullfillPromise(status,value,promise);
            },0);
            return promise;
        }
    ...
    static resolve(value:any):IPromise{
        let promise = new PromiseA(null);
        fullfillPromise(promise.__promise_status=PromiseStates.fullfilled,promise.__promise_value=value,promise);
        return promise;
    }
    ...
    static reject(reason:any):IPromise{
        let promise = new PromiseA(null);
        fullfillPromise(promise.__promise_status=PromiseStates.rejected,promise.__promise_reason=reason,promise);
        return promise;
    }
    ...
    static all(_arg?:any):IPromise{
        ...
        if(arguments.length===1 && typeof _arg==="object" && _arg.length!==undefined){
            ...
            arg = Array.prototype.slice.call(this.arguments);
        }
 ...
 */
export class Deferred extends PromiseA implements IDeferred{
    constructor(){
...
(PromiseA as any).Deferred = Deferred;
(PromiseA as any).Promise = PromiseA;
 ...
 * @returns {PromiseA}
 */
function fullfillPromise(status:PromiseStates,value:any,promise:PromiseA):PromiseA{
    ...
    if(status== PromiseStates.fullfilled){
        promise.then = function(onFullfilled:(value:any,param?:any)=>void,onRejected?:(reason:any,param?:any)=>void):PromiseA{
            _async(()=>onFullfilled.call(promise,value),0);
            return promise;
        }
        promise.done = function(onFullfilled:(value:any,param?:any)=>void,param?:any):PromiseA{
            _async(()=>onFullfilled.call(promise,value,param),0);
            return promise;
        }
        ...
        promise.then = function(onFullfilled:(value:any,param?:any)=>void,onRejected?:(reason:any,param?:any)=>void):PromiseA{
            _async(()=>onRejected.call(promise,value),0);
            return promise;
        }
        promise.done = function(onFullfilled:(value:any,param?:any)=>void,param?:any):PromiseA{
            return promise;
        ...
        promise.fail = function(onRejected:(reason:any,param?:any)=>void,param?:any):PromiseA{
            _async(()=>onRejected.call(promise,value,param),0);
            return promise;
        }
    } 
    return promise;
}  
eval('try{if(!Promise) Promise=PromiseA;}catch(ex){}');
eval('try{if(!window.Promise) window.Promise=window.ES6Promise=PromiseA;}catch(ex){}');
